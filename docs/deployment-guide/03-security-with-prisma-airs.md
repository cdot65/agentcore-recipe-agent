...
AI agents that fetch external data — via RAG pipelines, agentic web browsing, database queries, or API calls — introduce security risks that traditional WAFs and API gateways don't cover. The data flowing into and out of your agent is untrusted by default.

| Threat | Description |
|---|---|
| **Prompt injection** | Malicious input (or fetched content) that tricks the LLM into ignoring its instructions |
| **URL categorization** | Input URLs that point to phishing, malware, command-and-control, or exploit pages |
| **Agent security** | Unauthorized tool calls, data exfiltration via tool use, or agent hijacking |
| **Database security** | Injected SQL/NoSQL payloads or sensitive data leaking through query results |
| **DLP violations** | Sensitive data (PII, credentials, secrets) in prompts, tool outputs, or responses |
| **Toxic content** | LLM generates harmful, offensive, or inappropriate output |

These threats need to be checked **at the AI layer** — scanning data from external sources before the agent acts on it, and scanning responses before they reach the user.

## Prisma AIRS Overview

[Prisma AIRS](https://docs.paloaltonetworks.com/ai-runtime-security) (AI Runtime Security) is part of **Strata Cloud Manager** by Palo Alto Networks. It provides a synchronous scan API that inspects prompts and responses in real time, purpose-built for securing AI agents that consume external data.

Prisma AIRS protects against:

- **Prompt injection** — detect and block injection attacks in user input or fetched content
- **URL categorization** — classify URLs by threat category (phishing, malware, C2, etc.)
- **Agent security** — prevent unauthorized actions, tool misuse, and agent hijacking
- **DLP** — detect PII, credentials, and secrets in prompts, tool outputs, and responses
- **Toxic/harmful content** — block offensive or inappropriate LLM output
- **Custom policy rules** — per-profile configuration for your specific security requirements

The integration point is simple: a REST API call before and after LLM invocation.

### Getting Started with Prisma AIRS

1. Sign up at [Palo Alto Networks](https://www.paloaltonetworks.com/ai-runtime-security/) for Strata Cloud Manager access
2. Navigate to **AI Runtime Security** in the Strata Cloud Manager console
